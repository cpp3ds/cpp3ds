...
#ifndef EMULATION
    void sslc_init(cpp3ds::Socket::SecureData &data, int sockfd, const char *hostname)
    {
        // TODO: do error checking on return values
        Result ret = 0;
        ret = sslcCreateRootCertChain(&data.rootCertChain);
//        ret = sslcRootCertChainAddDefaultCert(data.rootCertChain, SSLC_DefaultRootCert_DigiCert_EV, NULL);
        ret = sslcOpenDefaultClientCertContext(SSLC_DefaultClientCert_ClCertA, &data.clientCertContext);
        ret = sslcCreateContext(&data.sslContext, sockfd, SSLCOPT_DisableVerify, hostname);
	    ret = sslcContextSetClientCert(&data.sslContext, data.clientCertContext);
	    ret = sslcContextSetRootCertChain(&data.sslContext, data.rootCertChain);
	    ret = sslcStartConnection(&data.sslContext, NULL, NULL);
    }
#endif
}
